...
if(isset($_GET['is_weight'])) $is_weight = $_GET["is_weight"];
...
fwrite($handle,'['.date('d/m/y h:i:s').'] --'.$isinit.'-'.$is_weight."\n");
...
//$sort_variableID = $variableID;
//sort($sort_variableID);
//fwrite($handle,'sort-'.implode(',',$sort_variableID).'-'."\n");

//$tempName = md5(rand());
$tempName = 'corre_v'.implode('v',$variableID).'_c'.$is_weight.'_'.$input_target.$catch_codename;
...
$r_intro_script = '';
$r_intro_script .= 'source("C:/AppServ/www/'.$RRoot.'R/f_Correlation.R")'."\n";
...
if($is_weight==0)
$r_intro_script .= "y=f_Correlation(data,0)\n";
if($is_weight==1)
$r_intro_script .= "y=f_Correlation(data,1)\n";
...
$CorrelationMatrix = $get_json->CorrelationMatrix;
$SumSquareMatrix = $get_json->SumSquareMatrix;
$CovarianceMatrix =  $get_json->CovarianceMatrix;
...
$question_label_array_length = count($question_label_array);
...
$CorrelationMatrix_array = array();
foreach( $CorrelationMatrix as $CorrelationMatrix_i ){
	array_push($CorrelationMatrix_array,fitdot($CorrelationMatrix_i,$dotmount));
}

$SumSquareMatrix_array = array();
foreach( $SumSquareMatrix as $SumSquareMatrix_i ){
	array_push($SumSquareMatrix_array,fitdot($SumSquareMatrix_i,$dotmount));
}

$CovarianceMatrix_array = array();
foreach( $CovarianceMatrix as $CovarianceMatrix_i ){
	array_push($CovarianceMatrix_array,fitdot($CovarianceMatrix_i,$dotmount));
}


$arraynew['DesStat'] = $DesStat_array;
$arraynew['CorrelationMatrix'] = $CorrelationMatrix_array;
$arraynew['SumSquareMatrix'] = $SumSquareMatrix_array;
$arraynew['CovarianceMatrix'] = $CovarianceMatrix_array;
$arraynew['state'] = 'ok';
