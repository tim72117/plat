...
app.controller('analysisController', function($scope, $filter, $interval, $http) {
    ...
    $scope.getFrequence = function(names, group_key, target_key) {  
        ...
        .success(function(data, status, headers, config) {
            console.log(data);
            if (!$scope.selected.columns[0].answers) {
                $scope.selected.columns[0].answers = [];
                for (var i in data.frequence) {
                    $scope.selected.columns[0].answers.push({title: i, value: i});
                }
            };
    ...
    $scope.getCrossTable = function(names, group_key, target_key) {  
        ...
        .success(function(data, status, headers, config) {
            ...
            if (!$scope.selected.columns[0].answers) {
                $scope.selected.columns[0].answers = [];
                for (var i in data.crosstable) {
                    $scope.selected.columns[0].answers.push({title: i, value: i});
                };
            };
            if (!$scope.selected.rows[0].answers) {
                $scope.selected.rows[0].answers = [];
                for (var i in data.crosstable) {
                    for (var j in data.crosstable[i]) {
                        if ($filter('filter')($scope.selected.rows[0].answers, {value: j}).length == 0) {
                            $scope.selected.rows[0].answers.push({title: j, value: j});
                        };                        
                    }                    
                }
            };
