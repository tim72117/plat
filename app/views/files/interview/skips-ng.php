                    ...
                    <div class="accordion">
                        ...
                        <div ng-repeat-end class="content" ng-class="{active:answer==selecting}">
                            ...
                            <div class="ui middle aligned divided list">
                                ...
                                <div class="item" ng-repeat="skip_ques in answer.rule.skip_question">
                                    <div class="ui mini green tag label icon">
                                        {{ skip_ques.is.title }}
                                        <i class="icon close" ng-click="deleteSkip(answer.rule.skip_question, skip_ques,'question')"></i>
                                    </div>
                                </div>
...
app.controller('editorController', function($http, $scope, $sce, $interval, $filter, $timeout) {
    ...
    $scope.deleteSkip = function(skip_questions, skip_question,type) {
        skip_questions.splice(skip_questions.indexOf(skip_question), 1);
           ...
           if ($scope.poolPage) {
               for (var i in $scope.poolPage) {
                   for (var j in $scope.poolPage[i].questions) {
                       if ($scope.poolPage[i].questions[j].id == skip_question.id) {
            ...
            if ($scope.poolQuestion) {
                for (var i in $scope.poolQuestion) {
                    for (var j in $scope.poolQuestion[i].answers) {
                        if ($scope.poolQuestion[i].answers[j].id == skip_question.id) {
    ...
    $scope.setRule = function(answer) {
        ...
        if (!$scope.selecting.rule) {
            $scope.selecting.rule = {skip_answers: [],skip_question: [],jump_book: [],open_wave: []};
        }
    ...
    $scope.setSkipQuestion = function(question) {
        var check_item = [];
        if (question.checked) {
            $scope.selecting.rule.skip_question.push(question);
            for(var i in question.answers) {
                question.answers[i].checked = false;
                $scope.setSkipAnswer(question.answers[i],question);
            }
        } else {
            for(key in $scope.selecting.rule.skip_question){
                if ($scope.selecting.rule.skip_question[key]['id'] == question['id']) {
                    delete $scope.selecting.rule.skip_question[key];
                } else {
                    check_item.push($scope.selecting.rule.skip_question[key]);
                }
            }
            $scope.selecting.rule.skip_question = check_item;
        }
    };
    ...
    $scope.toggle_pool = function(answer,skips,type,parent) {
        if (type == 'question') {
            ...
            if (answer.rule.skip_question) {
                var questions_id = [];
                for(key in answer.rule.skip_question){
                    questions_id.push(answer.rule.skip_question[key].id);
                }
