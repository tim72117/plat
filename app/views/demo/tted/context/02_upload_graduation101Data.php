...
##########################################################################################
#
# filename: upload_graduation101Data.php
# function: 上傳101學年度應屆畢業師資生資料
#
# 維護者  : 周家吉
# 維護日期: 2013/11/21
#
##########################################################################################
	...
	//資料庫連結，及存取之資料表
		...
		$funname ='edu_102/upload/upgraduation101Data_2003.php';
		$serverdir ='/home/leon/data/edu/data/graduation101/';
	
		$tb_name='[tted_edu_102].[dbo].[graduation101_userinfo] ';
...
<?		
	...
	if ($_FILES["sfile1"]["error"] == 0){	
			...
			Values ('".$sch_id."','".$name."','".$account."','".$memo."','".$contact."','".$sfilename."','".$ip."','3')";
	...
	for ($i = 4; $i <= $Import_NewRowCount; $i++) { 
		...
		*/		
		...
		if($datalist[19] == 0 || $datalist[19] ==1 || $datalist[19]==2 || $datalist[19]==5 || $datalist[19]==6){
				$newcid = createnewcid($datalist[9]); 
			}else if($datalist[19]==3|| $datalist[19]==4 || $datalist[19]==7 || $datalist[19]==8){
				$newcid = $datalist[9]; 
			}
		//echo $newcid;exit;
	...
	// 顯示欄位錯誤訊息結束-------------------------------------------------------------------				
			...
			//echo '增加'.'</br>' ;
				$InsertStr.="Insert into $tb_name([uid],[stdid],[yearenroll],[udepcode],[udepname],[stdschoolsys],[stdname],[stdidnumber],[stdemail],[stdregzipcode],[stdregaddr],[tel],[wayenroll],[childprogram],[priprogram],[secprogram],[speprogram],[other],[birthyear],[aboriginal],[gender],[newcid]) 
							values (".$value.")\n ;";	
				$InsertStr.="Insert into [tted_edu_102].[dbo].[graduation101_id] (stdidnumber,newcid) Values ('".$datalist[9]."','".$newcid."')\n ;";		
				$InsertStr.="Insert into [tted_edu_102].[dbo].[graduation101_pstat](newcid) Values ('".$newcid."')\n ;";				
				$insert_count +=1;			
			...
			//echo '刪除'.'</br>' ;
				...
				$InsertStr.="delete from [tted_edu_102].[dbo].[graduation101_id] where newcid='".$newcid."'\n";
				$InsertStr.="delete from [tted_edu_102].[dbo].[graduation101_pstat] where newcid='".$newcid."'\n";
				$delete_count +=1;
			}	
		}			
	}
...
<?php					
		...
		//寫入檔案中進行備份 Server
			...
			fclose($f);	
		//寫入檔案中進行log
			$filename = "upgraduation101_userinfoData_2003.log";
...
?>	
	<form id="form1" name="form1" method="post" action="graduation101.php">
