...
$contact = DB::table('ques_admin.dbo.contact')->where('user_id', $user->id)->where('project', 'tted');
$work = DB::table('ques_admin.dbo.work_tted')->where('user_id', $user->id)->first();
?>
<page pageset="old" backtop="7mm" backbottom="7mm" backleft="10mm" backright="10mm"> 
...
<table width="800" align="center" cellpadding="0" cellspacing="0" border="1" style="font-size:12pt; font-family:'標楷體'">
    <tr>
        ...
        <td width="296" height="50"><p align="center">
            <?
            User_tted::find($user->id)->schools->each(function($school){
...
<table width="800" align="center" cellpadding="0" cellspacing="0" border="1" style="font-family:'標楷體'">
  ...
  <tr>
    <td rowspan="3" style="center"><p><?=Form::radio('', '', $work->type == '1')?>教育部 </p>
      <p><?=Form::radio('', '', $work->type == '2')?>縣市政府承辦人 </p>
      <p><?=Form::radio('', '', $work->type == '0')?>師培大學承辦人 </p>
      <p><?=Form::radio('', '', $work->type == '3')?>其他 </p></td>
    <td><?=Form::checkbox('', '', $contact->exists())?>師資培育長期追蹤資料庫調查（含問卷查詢平台、線上分析系統等）</td>
