...
return array(
    'getUsers' => function() {        
        $cacheName = Auth::user()->id.'-school-profiile-users-tiped-1028';     

        Input::get('reflash', false) && Cache::forget($cacheName);

        $users = Cache::remember($cacheName, 3, function() {            
            return User_tiped::with(['contact', 'schools', 'departments'])->has('contact')->whereHas('schools', function($query) {
                $query->where('pub_school_u.id', '1028');
            })->where('id', '>', '20')->get();
        })->map(function($user){    
