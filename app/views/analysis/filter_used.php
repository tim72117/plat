<?php
return [
    'FW'             => 'FW_new',
    'my'             => ['name' => '本校', 'shid' => User_use::find(Auth::user()->id)->schools->map(function($school)
                                                    {
                                                        return $school->id;
                                                    })->all()
                        ],
    'groups' => [
        'all' => [
            'key'   => 'all',
            'name'  => '全國',
            'targets' => [
                'all'            => ['name' => '全國'],
                'all-state'      => ['name' => '全國國立學校', 'type1' => [1]],
                'all-private'    => ['name' => '全國私立學校', 'type1' => [2]],
                'all-county'     => ['name' => '全國縣市立學校', 'type1' => [3]],
            ],
        ],
        'state' => [
            'key'   => 'state',
            'name'  => '國立學校',
            'targets' => [
                'state-normal'   => ['name' => '國立高中', 'type1' => [1], 'type3' => 1],
                'state-skill'    => ['name' => '國立高職', 'type1' => [1], 'type3' => 2],
                'state-five'     => ['name' => '國立五專', 'type1' => [1], 'type3' => 3],
                'state-night'    => ['name' => '國立進校', 'type1' => [1], 'type3' => 4],
            ],
        ],
        'private' => [
            'key'   => 'private',
            'name'  => '私立學校',
            'targets' => [
                'private-normal' => ['name' => '私立高中', 'type1' => [2], 'type3' => 1],
                'private-skill'  => ['name' => '私立高職', 'type1' => [2], 'type3' => 2],
                'private-five'   => ['name' => '私立五專', 'type1' => [2], 'type3' => 3],
                'private-night'  => ['name' => '私立進校', 'type1' => [2], 'type3' => 4],
            ],
        ],
        'countys' => [
            'key'   => 'countys',
            'name'  => '縣市立學校',
            'targets' => [
                'countys-normal'  => ['name' => '縣市立高中', 'type1' => [3], 'type3' => 1],
                'countys-skill'   => ['name' => '縣市立高職', 'type1' => [3], 'type3' => 2],
                'countys-night'   => ['name' => '縣市立進校', 'type1' => [3], 'type3' => 4],
            ],
        ],
        'set' => [
            'key'   => 'set',
            'name'  => '公/私立學校',
            'targets' => [
                'set-public'      => ['name' => '公立學校', 'type2' => 1],
                'set-private'     => ['name' => '私立學校', 'type2' => 2],
            ],
        ],
        'mix' => [
            'key'   => 'mix',
            'name'  => '綜合高中',
            'targets' => [
                'mix-yes'         => ['name' => '綜合高中', 'type4' => 1],
                'mix-no'          => ['name' => '非綜合高中', 'type4' => 2],
            ],
        ],
        'citys' => [
            'key'   => 'citys',
            'name'  => '各縣市',
            'targets' => [
                'CR01'   => ['name' => '台北市', 'city1' => '30'],
                'CR02'   => ['name' => '新北市', 'city1' => '01'],
                'CR03'   => ['name' => '基隆市', 'city1' => '17'],
                'CR04'   => ['name' => '桃園縣', 'city1' => '03'],
                'CR05'   => ['name' => '新竹縣', 'city1' => '04'],
                'CR06'   => ['name' => '新竹市', 'city1' => '18'],
                'CR07'   => ['name' => '苗栗縣', 'city1' => '05'],
                'CR08'   => ['name' => '台中市', 'city1' => '66'],
                'CR09'   => ['name' => '彰化縣', 'city1' => '07'],
                'CR10'   => ['name' => '南投縣', 'city1' => '08'],
                'CR11'   => ['name' => '雲林縣', 'city1' => '09'],
                'CR12'   => ['name' => '嘉義縣', 'city1' => '10'],
                'CR13'   => ['name' => '嘉義市', 'city1' => '20'],
                'CR14'   => ['name' => '台南市', 'city1' => '67'],
                'CR15'   => ['name' => '高雄市', 'city1' => '64'],
                'CR16'   => ['name' => '屏東縣', 'city1' => '13'],
                'CR17'   => ['name' => '宜蘭縣', 'city1' => '02'],
                'CR18'   => ['name' => '花蓮縣', 'city1' => '15'],
                'CR19'   => ['name' => '台東縣', 'city1' => '14'],
                'CR20'   => ['name' => '金門縣', 'city1' => '71'],
                'CR21'   => ['name' => '連江縣', 'city1' => '72'],
                'CR22'   => ['name' => '澎湖縣', 'city1' => '16'],
            ],
        ],
        'no-test' => [
            'key'   => 'no-test',
            'name'  => '免試就學區',
            'targets' => [
                'NTR01'          => ['name' => '基北區', 'city2' => '1'],
                'NTR02'          => ['name' => '桃園區', 'city2' => '2'],
                'NTR03'          => ['name' => '竹苗區', 'city2' => '3'],
                'NTR04'          => ['name' => '中投區', 'city2' => '4'],
                'NTR05'          => ['name' => '嘉義區', 'city2' => '5'],
                'NTR06'          => ['name' => '彰化區', 'city2' => '6'],
                'NTR07'          => ['name' => '雲林區', 'city2' => '7'],
                'NTR08'          => ['name' => '台南區', 'city2' => '8'],
                'NTR09'          => ['name' => '高雄區', 'city2' => '9'],
                'NTR10'          => ['name' => '屏東區', 'city2' => '10'],
                'NTR11'          => ['name' => '台東區', 'city2' => '11'],
                'NTR12'          => ['name' => '花蓮區', 'city2' => '12'],
                'NTR13'          => ['name' => '宜蘭區', 'city2' => '13'],
                'NTR14'          => ['name' => '澎湖區', 'city2' => '14'],
                'NTR15'          => ['name' => '金門區', 'city2' => '15'],
            ],
        ],
    ],

    'county-my-state-normal' => ['name' => '本縣市國立高中', 'type1' => 1, 'type3' => 1, 'city1' => ''],
    'county-my-state-skill'  => ['name' => '本縣市國立高職', 'type1' => 1, 'type3' => 2, 'city1' => ''],
    'county-my-state-five'   => ['name' => '本縣市國立五專', 'type1' => 1, 'type3' => 3, 'city1' => ''],
    'county-my-state-night'  => ['name' => '本縣市國立進校', 'type1' => 1, 'type3' => 4, 'city1' => ''],	

    'county-my-private-normal' => ['name' => '本縣市私立高中', 'type1' => 2, 'type3' => 1, 'city1' => ''],
    'county-my-private-skill'  => ['name' => '本縣市私立高職', 'type1' => 2, 'type3' => 2, 'city1' => ''],
    'county-my-private-five'   => ['name' => '本縣市私立五專', 'type1' => 2, 'type3' => 3, 'city1' => ''],
    'county-my-private-night'  => ['name' => '本縣市私立進校', 'type1' => 2, 'type3' => 4, 'city1' => ''],	

    'county-my-county-normal' => ['name' => '本縣市縣市立高中', 'type1' => 3, 'type3' => 1, 'city1' => ''],
    'county-my-county-skill'  => ['name' => '本縣市縣市立高職', 'type1' => 3, 'type3' => 2, 'city1' => ''],
    'county-my-county-night'  => ['name' => '本縣市縣市立進校', 'type1' => 3, 'type3' => 4, 'city1' => ''],

    'county-my-public'  => ['name' => '本縣市公立學校', 'type2' => 1, 'city1' => ''],
    'county-my-private' => ['name' => '本縣市私立學校', 'type2' => 2, 'city1' => ''],

    'county-my-mix'    => ['name' => '本縣市綜合高中', 'type4' => 1, 'city1' => ''],
    'county-my-nmix'   => ['name' => '本縣市非綜合高中', 'type4' => 2, 'city1' => ''],

    'county-my'   => ['name' => '本縣市', 'city1' => ''],


];